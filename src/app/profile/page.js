...
const Profile = () => {
  ...
  const getUserById = async () => {
    ...
    try {
      const res = await Api.fnSendGetReq(
  ...
  const handleUpdate = async () => {
    try {
      ...
      appStore.dispatch({ type: "LOADER", payload: true });
      const res = await Api.fnSendPutReq(
        `std/update-std?id=${Cookies.getItem("id")}`,
        { data: dataObj }
      );
  ...
  const fnDelete = async () => {
    try {
      appStore.dispatch({ type: "LOADER", payload: true });
      const res = await Api.fnSendDeleteReq(
        `std/delete-std/${Cookies.getItem("id")}`
      );
  ...
  return (
    <div className="container-fluid">
      ...
      {inputControls?.map((obj, index) => {
        ...
        return (
          <div key={`div_${index}`} className="row mb-3">
      ...
      <div className="row">
        <div className="offset-sm-5 col-sm-7">
          ...
          <button onClick={handleTerminate} className="btn btn-primary me-3">
            Terminate
          </button>
        </div>
      </div>
